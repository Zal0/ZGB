#include "Flip.h"

// Constants for tile dimensions
#define TILE_WIDTH          8
#define TILE_HEIGHT         8
#define NUM_BYTES_PER_TILE  16

static UINT8 flipped_data[NUM_BYTES_PER_TILE];

// Table for fast reversing of bits in a byte - used for flipping in X
static const UINT8 reverse_bits[256] = {
	
};

// Helper function to flip tile in X/Y
// Note this assumes 2BPP tile in GB format, where bitplanes are interleaved.
// Currently all platforms use GB format for 2BPP tile data storage, irrespective
// of what their native tile format is, as set_sprite_data handles the conversion.
void set_flipped_tile(UINT8 tile_idx, UINT8* data, UINT8 flip) {
	for(UINT16 i = 0; i < TILE_HEIGHT; i++) {
		size_t y = (flip & FLIP_Y) ? (TILE_HEIGHT-1-i) : i;
		flipped_data[2*i] = (flip & FLIP_X) ? reverse_bits[data[2*y]] : data[2*y];
		flipped_data[2*i+1] = (flip & FLIP_X) ? reverse_bits[data[2*y+1]] : data[2*y+1];
	}
	set_sprite_data(tile_idx, 1, flipped_data);
}

void set_sprite_data_flip(UINT8 first_tile, UINT8 nb_tiles, UINT8 *data, UINT8 flip) {
	for(UINT8 i = 0; i < nb_tiles; i++) {
		set_flipped_tile(first_tile + i, data + (UINT16)(NUM_BYTES_PER_TILE * i), flip);
	}
}
