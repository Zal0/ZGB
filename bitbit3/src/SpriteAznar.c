...
void Update_SPRITE_AZNAR() {
	if(sprite_manager_current_sprite->flags & OAM_VERTICAL_FLAG) {
		//moving left
		sprite_manager_current_sprite->x -= 1 << delta_time;
		if(!scroll_collisions[GetScrollTile(((sprite_manager_current_sprite->x + sprite_manager_current_sprite->coll_x) >> 3), (sprite_manager_current_sprite->y >> 3) + 2u)]) {
			sprite_manager_current_sprite->flags = 0u;
		}
	} else {
		//moving right
		sprite_manager_current_sprite->x += 1 << delta_time;
		if(!scroll_collisions[GetScrollTile(((sprite_manager_current_sprite->x + sprite_manager_current_sprite->coll_x + sprite_manager_current_sprite->coll_w) >> 3), (sprite_manager_current_sprite->y >> 3) + 2u)]) {
